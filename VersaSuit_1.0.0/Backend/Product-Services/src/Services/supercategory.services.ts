...
{
  private SuperCategoryRepository = new SuperCategoryRepository();
  ...
  * @returns 
  */
  async GetByID(CompanyID: number, SuperCategoryID: number): Promise<SuperCategory | null> {

    //get the data from repository
    const dt = await this.SuperCategoryRepository.FetchByID(CompanyID, SuperCategoryID);
    //if there is not data return null
    if(!dt) return null;
